...
import { tanggalIndonesia, hitungUsia } from './helpers';
...
export function generateResepPDF(res: Response, data: DataResepPDF): void {
  const M  = 45;                          // margin
  ...
  res.setHeader(
    'Content-Disposition',
    `inline; filename="resep-${data.pasien.nomor_rm}-${Date.now()}.pdf"`
  );
  doc.pipe(res);
  ...
  doc.fillColor(C.accent).fontSize(16).font('Helvetica-Bold')
     .text(env.CLINIC_NAME, pL, hY, { width: lW, lineBreak: false });

  doc.fillColor(C.muted).fontSize(8.5).font('Helvetica')
     .text('Sistem Elektronik Healthcare Anda Terintegrasi', pL, hY + 22, { width: lW });
  doc.text(`${env.CLINIC_ADDRESS}  ·  Telp ${env.CLINIC_PHONE}`, pL, doc.y, { width: lW });
  ...
  const usia   = hitungUsia(data.pasien.tanggal_lahir);
  ...
  doc.text(`Tgl lahir: ${tanggalIndonesia(data.pasien.tanggal_lahir)} (${usia} th)`, pL, doc.y, { width: lW });
  ...
  doc.fillColor(C.dark).fontSize(11).font('Helvetica')
     .text(tanggalIndonesia(data.tanggal_kunjungan), rX, pSecY + 13, { width: rW, align: 'right' });
  ...
  data.obat.forEach((item, i) => {
    ...
    doc.text(item.jumlah != null ? String(item.jumlah) : '–',
             colX[4] + PAD, rowY + 6, { width: colW[4] - PAD, lineBreak: false });
  ...
  doc.fillColor(C.dark).fontSize(9.5).font('Helvetica')
     .text(tanggalIndonesia(data.tanggal_kunjungan), sigX, sigY, { width: sigW, align: 'center', lineBreak: false });
