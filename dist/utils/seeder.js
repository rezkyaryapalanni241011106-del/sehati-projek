...
async function seed() {
    ...
    console.log('\n[SEED] Mulai seeding...\n');
    // Hash password
    const hash = await bcrypt_1.default.hash(PASS, env_1.env.BCRYPT_ROUNDS);
    console.log(`[SEED] Password hash (cost=${env_1.env.BCRYPT_ROUNDS}): ${hash}\n`);
    // Truncate
    ...
    for (const t of ['Resep', 'Catatan_SOAP', 'Kunjungan', 'OTP', 'Jadwal_Praktek', 'Pasien', 'Users', 'Spesialisasi', 'ICD10']) {
        await database_1.pool.execute(`DELETE FROM ${t}`);
    }
    ...
    for (const staf of STAF) {
        const totpSecret = speakeasy_1.default.generateSecret({ name: `SEHATI:${staf.username}` });
        await database_1.pool.execute(`INSERT INTO Users
    ...
    for (const [id_dokter, hari, mulai, selesai, durasi, kuota] of jadwals) {
        await database_1.pool.execute('INSERT INTO Jadwal_Praktek (id, id_dokter, hari, jam_mulai, jam_selesai, durasi_menit, kuota) VALUES (UUID(), ?, ?, ?, ?, ?, ?)', [id_dokter, hari, mulai, selesai, durasi, kuota]);
    }
    ...
    for (const [kode, deskripsi, kategori] of icd10Data) {
        await database_1.pool.execute('INSERT IGNORE INTO ICD10 (kode, deskripsi, kategori) VALUES (?, ?, ?)', [kode, deskripsi, kategori]);
    }
