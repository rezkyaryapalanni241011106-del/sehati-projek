...
function buatSlot(jamMulai, offsetMenit) {
...
async function insertKunjungan(p) {
    ...
    await database_1.pool.execute(`INSERT INTO Kunjungan
...
// ─── Helper: insert SOAP ──────────────────────────────────────────────────────
async function insertSoap(kunjunganId, data) {
    const id = (0, uuid_1.v4)();
    ...
    await database_1.pool.execute(`INSERT INTO Catatan_SOAP
     ...
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`, [id, kunjunganId, data.subjektif,
...
// ─── Helper: insert resep ─────────────────────────────────────────────────────
async function insertResep(soapId, urutan, obat) {
    await database_1.pool.execute(`INSERT INTO Resep (id,id_soap,urutan,nama_obat,dosis,frekuensi,durasi,jumlah,cara_pakai)
     VALUES (UUID(),?,?,?,?,?,?,?,?)`, [soapId, urutan, obat.nama, obat.dosis, obat.frekuensi, obat.durasi, obat.jumlah, obat.cara]);
}
// ─── Main ─────────────────────────────────────────────────────────────────────
async function seed() {
    ...
    const [delRes] = await database_1.pool.execute(`DELETE FROM Pasien
     WHERE id IN (${ph(pIds.length)})
        OR nomor_rm  IN (${ph(pRms.length)})
        OR nik       IN (${ph(pNiks.length)})
        OR nomor_hp  IN (${ph(pHps.length)})`, [...pIds, ...pRms, ...pNiks, ...pHps]);
    console.log(`[SEED-HARIAN] Pasien lama dihapus : ${delRes.affectedRows} row`);
    for (const p of PASIEN) {
        await database_1.pool.execute(`INSERT INTO Pasien (id, nomor_rm, nama_lengkap, tanggal_lahir, jenis_kelamin, nomor_hp, nik, alamat)
       VALUES (?,?,?,?,?,?,?,?)`, [p.id, p.rm, p.nama, p.lahir, p.gender, p.hp, p.nik, p.alamat]);
    }
    await database_1.pool.execute('SET FOREIGN_KEY_CHECKS = 1');
    // Verifikasi
    const [verPasien] = await database_1.pool.execute(`SELECT COUNT(*) AS cnt FROM Pasien WHERE id IN (${ph(pIds.length)})`, pIds);
    const pasienCount = verPasien[0].cnt;
    console.log(`[SEED-HARIAN] Pasien : ${pasienCount}/${PASIEN.length} data terverifikasi`);
    // 2. Pastikan jadwal Dr. Budi ada untuk hari ini
    const [rowsBudi] = await database_1.pool.execute('SELECT id, jam_mulai FROM Jadwal_Praktek WHERE id_dokter=? AND hari=? AND status_aktif=1 LIMIT 1', [DR_BUDI, HARI_INI]);
    let jadwalBudiId;
    let budiMulai;
    if (rowsBudi.length === 0) {
        jadwalBudiId = (0, uuid_1.v4)();
        budiMulai = '08:00:00';
        await database_1.pool.execute('INSERT INTO Jadwal_Praktek (id,id_dokter,hari,jam_mulai,jam_selesai,durasi_menit,kuota) VALUES (?,?,?,?,?,?,?)', [jadwalBudiId, DR_BUDI, HARI_INI, '08:00:00', '12:00:00', 15, 8]);
    ...
    const [rowsSari] = await database_1.pool.execute('SELECT id, jam_mulai FROM Jadwal_Praktek WHERE id_dokter=? AND hari=? AND status_aktif=1 LIMIT 1', [DR_SARI, HARI_INI]);
    let jadwalSariId;
    let sariMulai;
    if (rowsSari.length === 0) {
        jadwalSariId = (0, uuid_1.v4)();
        sariMulai = '09:00:00';
        await database_1.pool.execute('INSERT INTO Jadwal_Praktek (id,id_dokter,hari,jam_mulai,jam_selesai,durasi_menit,kuota) VALUES (?,?,?,?,?,?,?)', [jadwalSariId, DR_SARI, HARI_INI, '09:00:00', '12:00:00', 20, 9]);
    ...
    const [existing] = await database_1.pool.execute('SELECT id FROM Kunjungan WHERE tanggal=?', [TODAY]);
    if (existing.length > 0) {
        const ids = existing.map((r) => r.id);
        const placeholders = ids.map(() => '?').join(',');
        await database_1.pool.execute(`DELETE FROM Resep WHERE id_soap IN (SELECT id FROM Catatan_SOAP WHERE id_kunjungan IN (${placeholders}))`, ids);
        await database_1.pool.execute(`DELETE FROM Catatan_SOAP WHERE id_kunjungan IN (${placeholders})`, ids);
        await database_1.pool.execute(`DELETE FROM Kunjungan WHERE tanggal=?`, [TODAY]);
    ...
    await database_1.pool.execute(`UPDATE Kunjungan SET status='selesai' WHERE id=?`, [k1]);
    ...
    await database_1.pool.execute(`UPDATE Kunjungan SET status='selesai' WHERE id=?`, [k2]);
    ...
    await database_1.pool.execute(`UPDATE Kunjungan SET status='selesai' WHERE id=?`, [k6]);
...
//# sourceMappingURL=seed-harian.js.map