...
exports.ResepModel = void 0;
const database_1 = require("../../config/database");
class ResepModel {
    async findResepData(soapId) {
        const [soaps] = await database_1.pool.execute(`SELECT cs.id, cs.tindakan, cs.anjuran, cs.kode_dx,
       ...
       WHERE cs.id = ? LIMIT 1`, [soapId]);
        if (soaps.length === 0)
            return null;
        const soap = soaps[0];
        const [obatRows] = await database_1.pool.execute('SELECT * FROM Resep WHERE id_soap = ? ORDER BY urutan', [soapId]);
        return {
            pasien: {
    ...
    async verifySoapMilikPasien(soapId, pasienId) {
        const [rows] = await database_1.pool.execute(`SELECT cs.id FROM Catatan_SOAP cs
       JOIN Kunjungan k ON cs.id_kunjungan = k.id
       WHERE cs.id = ? AND k.id_pasien = ? LIMIT 1`, [soapId, pasienId]);
        return rows.length > 0;
    }
}
exports.ResepModel = ResepModel;
//# sourceMappingURL=resep.model.js.map