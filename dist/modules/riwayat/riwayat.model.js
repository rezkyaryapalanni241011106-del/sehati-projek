...
exports.RiwayatModel = void 0;
const database_1 = require("../../config/database");
class RiwayatModel {
    async findKunjunganPasien(pasienId) {
       ...
       WHERE k.id_pasien = ?
       ORDER BY k.tanggal DESC, k.slot_jam DESC`, [pasienId]);
        return rows;
    }
    async findDetailKunjunganPasien(kunjunganId, pasienId) {
    ...
    async findSoap(kunjunganId) {
        const [rows] = await database_1.pool.execute(`SELECT cs.*, i.deskripsi AS dx_label
       FROM Catatan_SOAP cs
       LEFT JOIN ICD10 i ON i.kode = cs.kode_dx
       WHERE cs.id_kunjungan = ? LIMIT 1`, [kunjunganId]);
        return rows[0] ?? null;
    }
    async findResepBySoap(soapId) {
        const [rows] = await database_1.pool.execute('SELECT * FROM Resep WHERE id_soap = ? ORDER BY urutan', [soapId]);
        return rows;
    }
    async findPasienById(pasienId) {
        const [rows] = await database_1.pool.execute('SELECT id, nama_lengkap, nomor_rm, tanggal_lahir, jenis_kelamin, alergi, riwayat_kronis FROM Pasien WHERE id = ? LIMIT 1', [pasienId]);
        return rows[0] ?? null;
    }
    // Cek apakah dokter pernah menangani pasien ini (minimal 1 kunjungan selesai atau hadir)
    async findKunjunganDokterPasien(dokterId, pasienId) {
        const [[row]] = await database_1.pool.execute(`SELECT COUNT(*) AS jumlah FROM Kunjungan
       WHERE id_dokter = ? AND id_pasien = ? LIMIT 1`, [dokterId, pasienId]);
        return row.jumlah > 0;
    }
    async findKunjunganLengkap(pasienId) {
        const [rows] = await database_1.pool.execute(`SELECT k.id, k.tanggal, k.slot_jam, k.status, k.keluhan_awal,
              u.nama_lengkap AS nama_dokter,
...
exports.RiwayatModel = RiwayatModel;
//# sourceMappingURL=riwayat.model.js.map